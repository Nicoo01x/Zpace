...
| Installers | `npm run tauri:build:win` / `:mac` locally. A release: bump `version` in `package.json` and `src-tauri/tauri.conf.json`, write the notes, `gh release create vX.Y.Z --target main --title "Zpace X.Y.Z" --notes-file notes.md` — the tag it creates runs `.github/workflows/release.yml`, which uploads the signed installers for the three OSes and `latest.json` into that release (signing key secrets are set; the key is in `~/.tauri/`, never in the repo) |
...
- The repo is **public**; its GitHub releases are the downloads and the updater channel (`author.ts` → `REPO`; no separate downloads repo). `main` is protected by a ruleset: pull requests need the owner's review (CODEOWNERS) and the three-OS `build` checks, squash merge only, linear history, no force pushes; the owner (repository admin) bypasses it for direct pushes, and `v*` tags are locked the same way. Secret scanning with push protection, Dependabot (grouped weekly PRs), CodeQL and private vulnerability reporting are on — keep them on. Workflow tokens are read-only unless a job says otherwise.
