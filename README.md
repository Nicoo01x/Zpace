...
- **The island on your desktop** — flip a switch and the pill leaves the window: it floats at the top of the screen, over every app, and reads Windows itself — the toasts in the Action Center, what Spotify or a browser is playing (cover, controls), the app that just opened, the volume, the battery, every copy — next to what the agents are doing. Edge, size, look, what it announces and what it shows at rest are yours to set.
